...
# new_df = pd.read_csv('data/dados.csv')

# new_df = new_df.dropna()
# new_df = new_df.drop_duplicates()

# new_df_to_drop = [
#   'ISBN_13', 'ISBN_10', 'ano', 'resenha', 'editora',
#   'autor', 'genero', 'descricao', 'male', 'female',
#   'idioma', 'leram', 'lendo', 'relendo', 'avaliacao',
#   'querem_ler', 'abandonos', 'paginas'
# ]
# new_df.drop(new_df_to_drop, inplace=True, axis=1)
...
# print('hello', newColumnsList.index('Economia'))
...
def testing():
  ...
  return new_df
    # for i in 
    # new_df = new_df.append({'titulo': df['titulo'][existentIndex], "rating": 95, "genero": "M"}, ignore_index=True)
    # for column in newColumnsList:
    #   if column.upper() in df['genero'][existentIndex].upper():
    #     df[column][existentIndex] = 1
new_df = testing()
...
#     new_df = new_df.append({'titulo': df['titulo'][i], "rating": 95, "genero": "M"}, ignore_index=True)
#   st.dataframe(new_df)

# df_with_genre()
# for para tirar linhas com todas as colunas de genero com 0
